...
async function  createMarketplaceListing(page,browser){
    ...
    try{
        ...
        function getAppDataDir(appName = 'multiprof_newscrappy_usa') {
        ...
        const readFolderContents = () => {
            ...
            const txtFile = files.find(file => file.endsWith('.json'));
        ...
        const scrappedvehicledetails=JSON.parse(rootfoldertxtContent);

        console.log("\n \n here is the json data , \n" , scrappedvehicledetails.length);
        ...
        for(const scrappedvehicledetail of scrappedvehicledetails){
            ...
            const mileage= scrappedvehicledetail.mileage;
            const engine= scrappedvehicledetail.engine;
            const drivetrain= scrappedvehicledetail.drivetrain;
            const transmission= scrappedvehicledetail.transmission;
            const trime= scrappedvehicledetail.trim;
            const stock_number= scrappedvehicledetail.stock_number;
            const exterior_color= scrappedvehicledetail.exterior_color?.toLowerCase() || '';;
            const interior_color= scrappedvehicledetail.interior_color?.toLowerCase() || '';
            ...
            console.log("Here is the path name ", linkpath);

            // Split the pathname by '/' and filter out empty strings (from leading/trailing slashes or double slashes)
            const segments = linkpath.split('/').filter(segment => segment.length > 0);

            const rowmake=segments[1];
            const rowmodel=segments[2];

            console.log("Here is the Row Make ", rowmake, " And here is the row Model ", rowmodel);

            // Clean and format: replace hyphens with spaces and capitalize words
            const make = rowmake.replace(/-/g, ' ').split(/\s+/).map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
            const model = rowmodel.replace(/-/g, ' ').split(/\s+/).map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');

            const vehicledetail={
                title,make,model,YOM,price,vin,mileage,engine,drivetrain,transmission,trime,stock_number,exterior_color,interior_color,vehiclepath
            }



            vehiclesdetails.push(vehicledetail);
        }

        console.log("Here are the vehicle details  /n", vehiclesdetails);





        console.log("Images root folder path" , imagespaths)

        const vehicleimagesfolders=fs.readdirSync(imagespaths[0]);

        console.log("Here are the image paths \n ", vehicleimagesfolders);
        ...
        for(const productfolder of vehiclesdetails){

             //only wait for 15 seconds before beginning to post for the next product in our catalogue 
             console.log("waiting for 10 seconds before beginning to post for the next product in our catalogue"); 
            ...
            const listingType="Vehicle for sale";
            ...
            if(listingType==='Item for sale'){
                ...
                try{
                    ...
                    console.log("\n \n here is vehicle for sale ", "listingTypeButtons[i])");
                    // listingTypeButtons[i].click()

                    // const vehicleforsalebutton =
                    const vehicleforsalebutton =await page.locator('xpath=//div[@role="button"][.//span[text()="Vehicle for sale"]]');                
                    console.log("vehicleforsalebutton.count()", await vehicleforsalebutton.count())
                    
                    // await page.locator('span:has-text("o")').first().waitFor({ timeout: 120000 });

                    await vehicleforsalebutton.click();

                    // Assuming 'page' is your Playwright Page object
                    console.log("Waiting for 5 seconds...");
                    await page.waitForTimeout(10000); // Waits for 5000 milliseconds (5 seconds)
                    console.log("Done waiting.");


                    //Vehicle type combo 
                    await page.locator("xpath=//label[@role='combobox'][.//span[text()='Vehicle type']]").click();

                    await page.waitForTimeout(5000);

                    const selectanoptiondiv=await page.locator("xpath=//div[@aria-label='Select an option'][@role='listbox']");
                    console.log("selectanoptiondiv.count() " ,await selectanoptiondiv.count())

                    const vehicletypeooptions=await selectanoptiondiv.locator("xpath=.//div[@role='option']").all();
                    // console.log("\n \n const vehicletypeooptions count length" , await vehicletypeooptions.count());

                    // const vehicletypeoptionsarray=
                    for(const vehicletypeoption of vehicletypeooptions){
                        const span=await vehicletypeoption.locator("xpath=.//span")
                        // console.log(await span.textContent());
                        const spanText=await span.textContent();

                        if(spanText==="Car/Truck"){
                        // if(spanText==="Car/Truck"){
                            console.log("\n Found the Vehicle/Truck and Clicked it \n", await span.textContent());
                            await vehicletypeoption.click();
                            break;
                        }
                        else{
                            console.log("Another Vehicle Type Option", await span.textContent());
                            
                        }
                    }

                    console.log("Here is the vehicle details folder" , productfolder);

                    //Select the Images to use
                    // Start waiting for file chooser before clicking. Note no await.
                    const fileChooserPromise = page.waitForEvent('filechooser');
                    // await page.locator("xpath=//div[@role='button'][.//span[text()='or drag and drop']]").click();
                    
                    console.log("Now trying to upload the images...");
                    await page.locator("xpath=//div[@role='button' and .//span[text()='Add photos'] and .//span[text()='or drag and drop']]").click();


                    const fileChooser = await fileChooserPromise;

                    const imagepathen=path.join(`${imagespaths}`,productfolder.vehiclepath);


                    console.log(" imagepaths ", imagepathen);

                    const imagesarray=fs.readdirSync(`${imagepathen}`);

                    const individualimagepaths=[];

                    for(const imagepath of imagesarray){
                        const imagepathe=path.join(imagepathen,imagepath); 
                        console.log("here is the full path",  imagepathe);
                        individualimagepaths.push(imagepathe);
                    }
                    ...
                    await fileChooser.setFiles(individualimagepaths);

                    // Assuming 'page' is your Playwright Page object
                    console.log("Waiting for 5 seconds...after clickcing choosing images");
                    await page.waitForTimeout(5000); // Waits for 5000 milliseconds (5 seconds)
                    // console.log("Done waiting.");

                    //YOM section
                    await page.locator("xpath=//label[@role='combobox'][.//span[text()='Year']]").click();
                    
                    console.log("Waiting for 5 seconds...after clickcing Year of Manufacture ");
                    await page.waitForTimeout(2000);


                    // const year things continue here...
                    const selectyearoptiondiv=await page.locator("xpath=//div[@aria-label='Select an option'][@role='listbox']");
                    console.log("selectyearoptiondiv.count() " ,await selectanoptiondiv.count())

                    const vehicleyomoptions=await selectanoptiondiv.locator("xpath=.//div[@role='option']").all();
                    // console.log("\n \n const vehicletypeooptions count length" , await vehicletypeooptions.count());

                    // const vehicletypeoptionsarray=
                    for(const vehicletypeoption of vehicleyomoptions){
                        const span=await vehicletypeoption.locator("xpath=.//span")
                        // console.log(await span.textContent());
                        const spanText=await span.textContent();

                        if(spanText===productfolder.YOM){
                        // if(spanText==="Car/Truck"){
                            console.log("\n Found the YOM and Clicked it \n", await span.textContent());
                            await vehicletypeoption.click();
                            break;
                        }
                        else{
                            console.log("Another YOM Option", await span.textContent());
                            
                        }
                    }

                    console.log("Waiting for 5 seconds...after selecting Year of Manufacture ");
                    await page.waitForTimeout(2000);


                    //Make sections vehicle make
                    console.log("About to select car make")
                    await page.locator("xpath=//label[@role='combobox'][.//span[text()='Make']]").click();
                    
                    console.log("Waiting for 5 seconds...after clicking on Vehicle Make ");
                    await page.waitForTimeout(3000);

                    //Make sections vehicle make continue here...
                    const selectmakeoptiondiv=await page.locator("xpath=//div[@aria-label='Select an option'][@role='listbox']");
                    console.log("selectCarMakeoptiondiv.count() " ,await selectmakeoptiondiv.count())

                    const vehiclemakeoptions=await selectanoptiondiv.locator("xpath=.//div[@role='option']").all();
                    // console.log("\n \n const vehicletypeooptions count length" , await vehicletypeooptions.count());

                    let carMakeMatched=false;

                    // const vehicletypeoptionsarray=
                    for(const vehicletypeoption of vehiclemakeoptions){
                        if(carMakeMatched===true){
                            break
                        }
                        console.log("Car Make Matched in Beginning ", carMakeMatched);

                        const span=await vehicletypeoption.locator("xpath=.//span")
                        // console.log(await span.textContent());
                        const spanText=await span.textContent();

                        const refinedSpanText=spanText.replace(/-/g,' ').split(/\s+/).map(word=>word.toLowerCase()).join(' ');
                        console.log("refinedSpanText ", refinedSpanText)

                        const productToPostMake=productfolder.make;
                        const refinedProductToPostMakeArray=productToPostMake.split(/\s+/).map(word=>word.toLowerCase())

                        for(const eachsingleword of refinedProductToPostMakeArray){
                            if(refinedSpanText.includes(eachsingleword)){
                                console.log("\n Found the Matching CAR MAKE and Clicked it clicked is", spanText + " splited  word used ", eachsingleword);
                                carMakeMatched=true;                                
                                await vehicletypeoption.click();
                                break;
                            }
                            else{
                                console.log("Didn't Match car Make Another Option Span Text=", spanText + "Splitted word used= ",eachsingleword);
                    ...
                    console.log("Waiting for 5 seconds...after filling Vehicle Make ");
                    await page.waitForTimeout(3000);


                    //Model sections vehicle make
                    const modelinput=await page.locator("xpath=//label[.//span[text()='Model']]//input")

                    // Use the .type() method with the delay option
                    await modelinput.type(productfolder.model, { delay: 500 }); // Waits 2000ms (2 seconds) after each character

                    console.log("Waiting for 5 seconds...after filling Vehicle Model ");
                    await page.waitForTimeout(3000);



                    //Make sections vehicle make
                    const mileageinput=await page.locator("xpath=//label[.//span[text()='Mileage']]//input")

                    // Use the .type() method with the delay option
                    await mileageinput.type(productfolder.mileage, { delay: 500 }); // Waits 2000ms (2 seconds) after each character


                    console.log("Waiting for 5 seconds...after filling Vehicle Mileage ");
                    await page.waitForTimeout(3000);



                    //Make sections vehicle make
                    const priceinput=await page.locator("xpath=//label[.//span[text()='Price']]//input")

                    // Use the .type() method with the delay option
                    await priceinput.type(productfolder.price, { delay: 500 }); // Waits 2000ms (2 seconds) after each character


                    console.log("Waiting for 5 seconds...after filling Vehicle Price ");
                    await page.waitForTimeout(5000);


                    //Body Type section 
                    await page.locator("xpath=//label[@role='combobox'][.//span[text()='Body style']]").click();
                    
                    console.log("Waiting for 5 seconds...after clickcing Vehicle Body Style ");
                    await page.waitForTimeout(5000);

                    const selectbodytypeoptiondiv=await page.locator("xpath=//div[@aria-label='Select an option'][@role='listbox']");
                    console.log("selectbodytypeoptiondiv.count() " ,await selectbodytypeoptiondiv.count())

                    const vehiclebodytypeoptions=await selectbodytypeoptiondiv.locator("xpath=.//div[@role='option']").all();
                    // console.log("\n \n const vehicletypeooptions count length" , await vehicletypeooptions.count());
                    ...
                    // const vehicletypeoptionsarray=
                    for(const vehicletypeoption of vehiclebodytypeoptions){    
                        ...
                        const fbSpanText = spanText.toLowerCase().trim();
                        const productTrimText = productfolder.trime.toLowerCase().trim();



                        // 1. Function to normalize strings for consistent comparison (lowercase, trim whitespace)
                        function normalizeText(text) {
                            return text ? String(text).toLowerCase().trim() : '';
                        }

                         // 2. Mapping of common body type synonyms/variations found on webpages
                        //    Key: Your normalized productfolder.trim value (e.g., 'sport utility vehicle')
                        //    Value: An array of normalized possible spanText values you might encounter on the page
                        const productTrimToNormalizedMap = {
                            'sedan': ['sedan', 'sedan (4-door)', '4-door sedan'],
                            'coupe': ['coupe', '2-door coupe'],
                            'convertible': ['convertible', 'soft top', 'hard top convertible', 'cabriolet'],
                            'hatchback': ['hatchback', '5-door hatchback', '3-door hatchback'],
                            'wagon': ['wagon', 'station wagon'],
                            'minivan': ['minivan', 'passenger van'],
                            'pickup truck': ['pickup truck', 'truck', 'pick up'],
                            'sport utility vehicle': ['suv', 'sport utility vehicle', 'crossover suv', 'compact suv', 'sport wagon', 'off-road vehicle'],
                            'sports car': ['sports car', '2-door sport', 'sport'], // If 'Sport' in your data truly means a dedicated sports car
                            'suv': ['sport utility vehicle', "suv", "sport utility", "sport", "RT","LT"],   // If your JSON has 'SUV' but you want to treat it as 'sport utility vehicle' internally
                            'truck': ['pickup truck',"truck"],          // Example
                            'small car':["small car", "car"],
                            'other':["other"],
                            // Add more mappings as you discover variations on the website
                     
                     
                        };

                        const normalizedFbBodyType = normalizeText(productTrimToNormalizedMap[normalizeText(fbSpanText)] || fbSpanText);

                         const splitbodytypestrings=productTrimText.split(" ").map(word => normalizeText(word)); // Normalize each word

                         console.log("splitbodytypestings ", splitbodytypestrings);

                         for(const word of splitbodytypestrings){

                            if (normalizedFbBodyType.includes(word)) {
                                console.log(`Found a match: "${normalizedFbBodyType}" contains "${productTrimText}"`);
                                console.log("\n Found the Body type and Clicked it \n", await span.textContent());
                                isMatch="true";
                                ...
                                console.log(`No direct inclusion match in "${normalizedFbBodyType}" of "${productTrimText}"`);
                            }
                    ...
                    await page.locator("xpath=//label[@role='combobox'][.//span[text()='Exterior color']]").click();
                    
                    console.log("Waiting for 5 seconds...after clicking Exterior color ");
                    await page.waitForTimeout(5000);


                    // const year things continue here...
                    const selectExteriocoloroptiondiv=await page.locator("xpath=//div[@aria-label='Select an option'][@role='listbox']");
                    console.log("selectExteriocoloroptiondiv.count() " ,await selectExteriocoloroptiondiv.count())

                    const vehicleExteriorColoroptions=await selectanoptiondiv.locator("xpath=.//div[@role='option']").all();
                    // console.log("\n \n const vehicletypeooptions count length" , await vehicletypeooptions.count());

                    const vehicleExteriorColoroptionslength=await selectExteriocoloroptiondiv.count();

                    // const vehicletypeoptionsarray=
                    for(let i=0; i<vehicleExteriorColoroptionslength;i++){
                        const span=await vehicleExteriorColoroptions[i].locator("xpath=.//span")
                        // console.log(await span.textContent());
                        const spanText=await span.textContent();
                        const lowerCaseExteriorColor=spanText.toLowerCase();


                        if(lowerCaseExteriorColor===productfolder.exterior_color){
                            console.log("\n Found the Exterior Color and Clicked it \n", await span.textContent(), "Matched Color = ", productfolder.exterior_color);
                            await vehicleExteriorColoroptions[i].click();
                            break;
                        }
                        else{
                            if(i===vehicleExteriorColoroptionslength-1){
                                console.log("Looped all Exterior colors No match so picked Default Exerior Color Black and Clicked",);
                                await vehicleExteriorColoroptions[0].click();
                                break; 
                            }
                            
                            console.log("NO Matched Another Exterior Color Option", await span.textContent(), "Tried Matching Color = ", productfolder.exterior_color);

                        }
                    }

                    console.log("Waiting for 5 seconds...after selecting Exterior Color ");
                    await page.waitForTimeout(2000);


                    //Interior Color section
                    await page.locator("xpath=//label[@role='combobox'][.//span[text()='Interior color']]").click();
                    
                    console.log("Waiting for 5 seconds...after clicking Interior color ");
                    await page.waitForTimeout(2000);


                    // const year things continue here...
                    const selectInteriorcoloroptiondiv=await page.locator("xpath=//div[@aria-label='Select an option'][@role='listbox']");
                    console.log("selectInteriorcoloroptiondiv.count() " ,await selectInteriorcoloroptiondiv.count())

                    const vehicleInteriorColoroptions=await selectanoptiondiv.locator("xpath=.//div[@role='option']").all();
                    // console.log("\n \n const vehicletypeooptions count length" , await vehicletypeooptions.count());

                    const vehicleInteriorColoroptionslength=await selectInteriorcoloroptiondiv.count();

                    // const vehicletypeoptionsarray=
                    for(let i=0; i<vehicleInteriorColoroptionslength;i++){
                        const span=await vehicleInteriorColoroptions[i].locator("xpath=.//span")
                        // console.log(await span.textContent());
                        const spanText=await span.textContent();
                        const lowerCaseInteriorColor=spanText.toLowerCase();

                        if(lowerCaseInteriorColor===productfolder.interior_color){
                            console.log("\n Found the Interior Color and Clicked it \n", await span.textContent(), "Matched Color = ", productfolder.interior_color);
                            await vehicleInteriorColoroptions[i].click();
                            break;
                        }
                        else{
                            if(i===vehicleInteriorColoroptionslength-1){
                                console.log("Looped all Interior colors No match so picked Default Interior Color: Gray and Clicked",);
                                await vehicleInteriorColoroptions[5].click();
                                break; 
                            }
                            
                            console.log("NO Matched Another Interior Color Option", await span.textContent(), "Tried Matching Color = ", productfolder.interior_color);

                        }
                    }

                    console.log("Waiting for 5 seconds...after selecting Interior Color ");
                    await page.waitForTimeout(2000);


                    //click on the This vehicle has a clean title. button
                    await page.locator("xpath=//input[@aria-label='This vehicle has a clean title.'][@type='checkbox']").click();


                    console.log("Waiting for 5 seconds...after clicking This vehicle has a clean title. button");
                    await page.waitForTimeout(2000);
                    


                    //Vehicle condition Handled here 
                    await page.locator("xpath=//label[@role='combobox'][.//span[text()='Vehicle condition']]").click();
                    
                    console.log("Waiting for 5 seconds...after clickcing Vehicle condition ");
                    await page.waitForTimeout(2000);


                    const selectvehicleconditionoptiondiv=await page.locator("xpath=//div[@aria-label='Select an option'][@role='listbox']");
                    console.log("selectVehicle conditionoption.count() " ,await selectanoptiondiv.count())

                    const vehicleconditionoptions=await selectanoptiondiv.locator("xpath=.//div[@role='option']").all();
                    // console.log("\n \n const vehicletypeooptions count length" , await vehicletypeooptions.count());
                    ...
                    for(const vehicletypeoption of vehicleyomoptions){
                        ...
                        if(spanText==="Excellent"){
                        // if(spanText==="Car/Truck"){
                            console.log("\n Found the Excellent Condition and Clicked it \n", await span.textContent());
                            await vehicletypeoption.click();
                            break;
                        }
                        else{
                            console.log("Another Excellent Condition Option", await span.textContent());
                            
                        }
                    }
                    

                    console.log("Waiting for 5 seconds...after filling Vehicle Condtion ");
                    await page.waitForTimeout(2000);
                    

                    //Vehicle Description Section 
                    console.log("Started the Vehicle Description Section....");
                    await page.waitForTimeout(2000);
                    
                    const descriptioninput=await page.locator("xpath=//label[.//span[text()='Description']]//textarea")

                    console.log(productfolder);
                    // Use the .type() method with the delay option

                    const vehicletemplateliterals=[
                        productfolder.title,
                        productfolder.price,
                        productfolder.vin,
                        productfolder.engine,
                        productfolder.drivetrain,
                        productfolder.transmission,
                        productfolder.trime,
                        productfolder.stock_number,
                        productfolder.exterior_color,
                        productfolder.interior_color,
                        productfolder.vehiclepath
                    ]
                    

                    for(let i=0; i<vehicletemplateliterals.length;i++){
                        await descriptioninput.type(vehicletemplateliterals[i], { delay: 100,timeout:120000}); // Waits 2000ms (2 seconds) after each character
                        // In listonmarketplace.js, around line 2605-2606
                        console.log("About to press key. Key value:", "Enter", "Type of key:", typeof "Enter");
                        await page.keyboard.press("Enter");
                        // await page.press("Enter")
                    }


                    console.log("Waiting for 5 seconds...after filling Vehicle Description ");
                    ...
                    ///**************Waiting and Now on the Next Button Section**************
                    try{
                        await page.waitForSelector("div[aria-label=Next]");
                    ...
                    ///**************Publish Button Section Waiting and Now on the Publish Button Section**************
                    try{
                    ...
                    console.log("Waiting for 5 seconds before going to the next product....");
                    await page.waitForTimeout(5000);
